...
namespace fl {
	...
	int g=0;
        ...
        printf("\n%d rules created\n",q);
	system("touch /home/root/Desktop/res_vec1.txt");
	//FILE *fo=fopen("res_vec.txt","w");
	std::ofstream fout("/home/root/Desktop/res_vec1.txt");
        fl::flScalar cp=0.0,cp1=0.0,cp2=0.0;
        //for (fl::flScalar in = 0.0; in < 1.1; in += 0.1) {
         while(g<30)
         {
            cp=exec("xentop -b -i 1  | grep 'fedora' | cut -f 19 -d ' '");   // hard coded
            i0->setInput(cp/100);
            sleep(sample_interval);
            cp1=exec("xentop -b -i 1 | grep 'fedora'| cut -f 19 -d ' '");
            i1->setInput(cp1/100);
            sleep(sample_interval);
            cp2=exec("xentop -b -i 1 | grep 'fedora'| cut -f 19 -d ' '");
            i2->setInput(cp2/100);
            engine.process();
